...
uint64_t rank9sel::select( const uint64_t rank ) {
	...
	register uint64_t block_left = inventory_left / 64;
	const uint64_t span = block_right / 4 - block_left / 4;
	const uint64_t * const s = &subinventory[ block_left / 4 ];
	register uint64_t count_left, rank_in_block;

#ifdef DEBUG
	fprintf(stderr, "Initially, rank: %lld block_left: %lld block_right: %lld span: %lld\n", rank, block_left, block_right, span );
#endif
	...
	if ( span < 2 ) {
...
#ifdef DEBUG
		fprintf(stderr, "Single span; rank_in_block: %lld block_left: %lld\n", rank_in_block, block_left );
#endif
#ifdef COUNTS
		single++;
#endif
		...
		assert( where <= 16 );
#ifdef DEBUG
		fprintf(stderr, "rank_in_superblock: %lld (%llx) %llx %llx\n", rank_in_superblock, rank_in_superblock, s[0], s[1] );
#endif
...
#ifdef DEBUG
		fprintf(stderr, "Found where (1): %d rank_in_block: %lld block_left: %lld\n", where, rank_in_block, block_left );
		fprintf(stderr, "supercounts: %016llx %016llx\n", s[0], s[1] );
#endif
#ifdef COUNTS
		one_level++;
#endif
...
#ifdef DEBUG
		fprintf(stderr, "Found where (2): %d rank_in_block: %lld block_left: %lld\n", where1, rank_in_block, block_left );
#endif
#ifdef COUNTS
		two_levels++;
#endif
...
#ifdef DEBUG
	fprintf(stderr, "rank_in_block: %lld offset_in_block: %lld rank_in_word: %lld compare: %016llx shift: %lld\n", rank_in_block, offset_in_block, rank_in_word, UCOMPARE_STEP_9( rank_in_block_step_9, subcounts ), subcounts >> ( offset_in_block - 1 ) * 9 & 0x1FF );
#endif
	assert( offset_in_block >= 0 );
	assert( offset_in_block <= 7 );

#ifdef DEBUG
	fprintf(stderr, "rank_in_block: %lld offset_in_block: %lld rank_in_word: %lld\n", rank_in_block, offset_in_block, rank_in_word );
	fprintf(stderr, "subcounts: " );
	for( int i = 0; i < 7; i++ ) fprintf(stderr, "%lld ", subcounts >> i * 9 & 0x1FF );
	fprintf(stderr, "\n" );
	fflush( stdout );
#endif
