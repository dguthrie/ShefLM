...
void brz_config_destroy(cmph_config_t *mph)
{
	...
	free(data->tmp_dir);
	printf("Destroying algorithm dependent data\n");
	free(data);
}
...
cmph_t *brz_new(cmph_config_t *mph, double c)
{
	...
	while(1)
	{
		...
		if (!ok)
		{
			...
			brz->h0 = NULL;
			printf("%u iterations remaining to create the graphs in a external file\n", iterations);
			if (mph->verbosity)
			{
	...
	if (iterations == 0) 
	{
		printf("Graphs with more than 255 keys were created in all 20 iterations\n");
		free(brz->size);
		return NULL;
	}
	...
	mphf->data = brzf;
	mphf->size = brz->m;	
	printf("Successfully generated minimal perfect hash\n");
	if (mph->verbosity)
	{
...
static int brz_gen_mphf(cmph_config_t *mph)
{
	...
	printf("Generating graphs from %u keys\n", brz->m);
	// Partitioning
	for (e = 0; e < brz->m; ++e)
	{
...
int brz_dump(cmph_t *mphf, FILE *fd)
{
	...
	// Dumping h0
        ...
        printf("Dumping hash state with %u bytes to disk\n", buflen);
...
void brz_load(FILE *f, cmph_t *mphf)
{
	...
	for(i = 0; i < brz->k; i++)
	{
		...
		//h2
		nbytes = fread(&buflen, sizeof(cmph_uint32), (size_t)1, f);
		printf("Hash state 2 has %u bytes\n", buflen);
		buf = (char *)malloc((size_t)buflen);
		nbytes = fread(buf, (size_t)buflen, (size_t)1, f);
...
static cmph_uint32 brz_bmz8_search(brz_data_t *brz, const char *key, cmph_uint32 keylen, cmph_uint32 * fingerprint)
{
	...
	printf("key: %s h1: %u h2: %u h0: %u\n", key, h1, h2, h0);
	printf("key: %s g[h1]: %u g[h2]: %u offset[h0]: %u edges: %u\n", key, brz->g[h0][h1], brz->g[h0][h2], brz->offset[h0], brz->m);
