...
chd_ph_bucket_t * chd_ph_bucket_new(cmph_uint32 nbuckets)
{
	...
	fprintf(stderr,"\nallocated %f GB of memory for buckets",nbuckets*sizeof(chd_ph_bucket_t)/1024.0/1024.0/1024.0);
    return buckets;
}
...
chd_ph_config_data_t *chd_ph_config_new()
{
	...
	fprintf(stderr,"\nallocated %f GB of memory for chd_ph_config_data_t",sizeof(chd_ph_config_data_t)/1024.0/1024.0/1024.0);
...
void chd_ph_config_destroy(cmph_config_t *mph)
{
	...
	fprintf(stderr,"Destroying algorithm dependent data\n");
	if(data->occup_table)
	{
...
cmph_uint8 chd_ph_mapping(cmph_config_t *mph, chd_ph_bucket_t * buckets, chd_ph_item_t * items, cmph_uint32 *max_bucket_size)
{
	...
	if (map_items==0) fprintf(stderr,"\ncould not allocate %u items of size %lu\n",chd_ph->m,sizeof(chd_ph_map_item_t));
	else fprintf(stderr, "\nSuccessfully allocated %f GB bytes for map items\n",chd_ph->m*sizeof(chd_ph_map_item_t)/1024.0/1024.0/1024.0);
...
cmph_t *chd_ph_new(cmph_config_t *mph, double c)
{
	...
	fprintf(stderr,"\nallocated %f GB of memory for items",chd_ph->m*sizeof(chd_ph_item_t)/1024.0/1024.0/1024.0);
	...
	if(chd_ph->keys_per_bin == 1){
		...
		fprintf(stderr,"\nallocated %f GB of memory for occup_table",chd_ph->n*sizeof(cmph_uint8)/1024.0/1024.0/1024.0);
	}
	disp_table = (cmph_uint32 *) calloc(chd_ph->nbuckets, sizeof(cmph_uint32));
	fprintf(stderr,"\nallocated %f GB of memory for occup_table\n",chd_ph->nbuckets*sizeof(cmph_uint32)/1024.0/1024.0/1024.0);
// 	
// 	init_genrand(time(0));
