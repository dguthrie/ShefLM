...
cmph_t *bdz_new(cmph_config_t *mph, double c)
{
	...
	printf("ranktablesize: %u\n", bdz->ranktablesize);

	
	bdz_alloc_graph3(&graph3, bdz->m, bdz->n);
	bdz_alloc_queue(&edges,bdz->m);
	printf("Created hypergraph\n");
	
	printf("m (edges): %u n (vertices): %u  r: %u c: %f \n", bdz->m, bdz->n, bdz->r, c);
